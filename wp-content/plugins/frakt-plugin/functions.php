			...
			class WC_Your_Shipping_Method extends WC_Shipping_Method
			{
				...
				function init_form_fields()
				{
					...
					foreach ($shipping_classes as $shipping_class) {
						...
						$test[$shipping_class->term_id] = array(
							'title'             => sprintf(__('"%s"pris utanför fraktzon är sum x vikt', 'your_shipping_method'), esc_html($shipping_class->name)),
							'type'              => 'number',
							'placeholder'       => __('pris utanför fraktzon är sum x vikt', 'your_shipping_method'),
							'description'       => __('See "Cost" option description above for available options.', 'your_shipping_method'),
							'default'           => $this->get_option('class_cost_' . $shipping_class->slug),
							'desc_tip'          => true,
						...
						$this->form_fields = array_merge($fleroptions, $pris, $vikt, $test);
						/* $current_user = wp_get_current_user();
							echo '<pre>';
							var_dump(get_user_meta( $current_user->ID, 'shipping_city', true )); */
						/* echo '<pre>';
				...
				public function check_distance()
				{
					...
					if ($shipping_city == "stockholm" || $shipping_city == "göteborg" || $shipping_city == "malmö" || $shipping_city == "lund") {
						return true;
					} else {
						return false;
					};
				...
				public function calculate_shipping($package = array())
				{
					...
					if($toFarAway == false){
						if($productsweight <= $this->settings[3]){
							$cost = $this->settings[6] * $this->settings[3];
						}else if($productsweight <= $this->settings[4]){
							$cost = $this->settings[7] * $this->settings[4];
						}else if($productsweight <= $this->settings[5]){
							$cost = $this->settings[8] * $this->settings[5] ;
						}else{
							$cost = $this->toMuch;
						}
					}
